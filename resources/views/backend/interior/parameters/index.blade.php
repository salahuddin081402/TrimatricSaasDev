...
@php
    ...
    /* --- patch: guards for optional vars (lists + collections) --- */
    $lists          = $lists          ?? [];
    $projectTypes    = $projectTypes    ?? collect();
    $projectSubtypes = $projectSubtypes ?? collect();
...
// Prefer named route if available; fallback to url() to avoid route exceptions.
    ...
    // Base URL for CRUD endpoints (index/store share same URI; update/destroy append /{id})
    $baseUrl = rtrim(url()->current(), '/');

    $initialEntity = old('_entity', $activeEntity ?? 'project_type');
    ...
    $entityMetaMap = [
        ...
        'space_category_mapping' => [
            ...
            'title'    => 'Space Categories Mappings · Create / Edit',
            'subtitle' => 'Map each Space to one or more allowed Item Categories (tenant scoped).',
        ],
        'subcategory' => [
                ...
                <aside class="ip-card">
                    ...
                    <div class="ip-card-body">
                        <div class="ip-entity-list" id="entityList">
                            ...
                            <button type="button"
                                    class="ip-entity-pill {{ $initialEntity === 'space_category_mapping' ? 'active' : '' }}"
                                    data-entity="space_category_mapping">
                                <span>
                                    <span class="ip-entity-pill-dot"></span>
                                    <span>Space Categories Mappings</span>
                                </span>
                                <small>(cr_space_category_mappings)</small>
                            </button>
                            ...
                            <aside class="ip-list-panel">
                                ...
                                <div class="ip-record-sets" id="recordSets">
                                    ...
                                    <div class="ip-record-set {{ $initialEntity === 'project_type' ? 'active' : '' }}"
                                         data-entity="project_type">
                                        @php $rows = collect($lists['project_type'] ?? []); @endphp
                                        @if($rows->isEmpty())
                                    ...
                                    <div class="ip-record-set {{ $initialEntity === 'project_subtype' ? 'active' : '' }}"
                                         data-entity="project_subtype">
                                        @php $rows = collect($lists['project_subtype'] ?? []); @endphp
                                        @if($rows->isEmpty())
                                    ...
                                    <div class="ip-record-set {{ $initialEntity === 'space' ? 'active' : '' }}"
                                         data-entity="space">
                                        @php $rows = collect($lists['space'] ?? []); @endphp
                                        @if($rows->isEmpty())
                                    ...
                                    <div class="ip-record-set {{ $initialEntity === 'category' ? 'active' : '' }}"
                                         data-entity="category">
                                        @php $rows = collect($lists['category'] ?? []); @endphp
                                        @if($rows->isEmpty())
                                    ...
                                    <div class="ip-record-set {{ $initialEntity === 'space_category_mapping' ? 'active' : '' }}"
                                         data-entity="space_category_mapping">
                                        @php $rows = collect($lists['space_category_mapping'] ?? []); @endphp
                                        @if($rows->isEmpty())
                                            <div class="ip-record-empty">No space-category mappings found.</div>
                                        @else
                                            <div class="ip-record-list">
                                                @foreach($rows as $row)
                                                    @php
                                                        $metaText = ($row->space_code ?? ('#'.$row->space_id)) . ' → ' . ($row->category_code ?? ('#'.$row->category_id));
                                                        $titleText = ($row->space_name ?? ('Space #'.$row->space_id)) . ' → ' . ($row->category_name ?? ('Category #'.$row->category_id));
                                                        $subText = $row->is_active ? 'Active' : 'Inactive';
                                                    @endphp
                                                    <button type="button"
                                                            class="ip-record-item"
                                                            data-entity="space_category_mapping"
                                                            ...
                                                            data-is-active="{{ (int)$row->is_active }}"
                                                            data-sort-order="{{ $row->sort_order }}"
                                                            data-space-label="{{ $row->space_name ?? '' }}"
                                                            data-category-label="{{ $row->category_name ?? '' }}">
                                                        <div class="ip-record-item-left">
                                                            <div class="ip-record-thumb ip-record-thumb--placeholder">
                                                                <span class="ip-record-thumb-text">SC</span>
                                                            </div>
                                                            <div class="ip-record-meta">
                                                                <div class="ip-record-name">{{ $titleText }}</div>
                                                                <div class="ip-record-code">{{ $metaText }} · {{ $subText }}</div>
                                                            </div>
                                                        </div>
                                                        <div class="ip-record-item-right">
                                                            <span class="ip-record-chevron">›</span>
                                                        </div>
                                                    </button>
                                                @endforeach
                                            </div>
...
<div class="ip-record-set {{ $initialEntity === 'subcategory' ? 'active' : '' }}"
                                         data-entity="subcategory">
                                        @php $rows = collect($lists['subcategory'] ?? []); @endphp
                                        @if($rows->isEmpty())
                                    ...
                                    <div class="ip-record-set {{ $initialEntity === 'product' ? 'active' : '' }}"
                                         data-entity="product">
                                        @php $rows = collect($lists['product'] ?? []); @endphp
                                        @if($rows->isEmpty())
                                                ...
                                                @foreach($rows as $row)
                                                    @php
                                                        $thumb = $row->main_image_url
                                                            ...
                                                            : null;
                                                        $subRows = $lists['subcategory'] ?? collect();
                                                        $subName = optional($subRows->firstWhere('id', $row->subcategory_id))->name ?? '';
...
<input type="checkbox"
                                                               id="pt_is_active"
                                                               name="is_active"
                                                               @if(old('is_active', '1')) checked @endif value="1">
...
<input type="checkbox"
                                    ...
                                    <!-- Space Categories Mappings -->
                                    <div class="ip-entity-form {{ $initialEntity === 'space_category_mapping' ? 'active' : '' }}"
                                         data-entity="space_category_mapping">
                                        <div class="ip-form-grid">
                                            <div class="ip-form-group">
                                                <label class="ip-label" for="scm_space_id">
                                                    Space <span class="required">*</span>
                                                </label>
                                                @php $spaceRows = collect($lists['space'] ?? []); @endphp
                                                <select id="scm_space_id" name="space_id" class="ip-field ip-select">
                                                    <option value="">-- Select Space --</option>
                                                    @foreach($spaceRows as $sp)
                                                        <option value="{{ $sp->id }}"
                                                            {{ (string)old('space_id') === (string)$sp->id ? 'selected' : '' }}>
                                                            {{ $sp->code }} · {{ $sp->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                <div class="ip-help">Space from cr_spaces (tenant scoped).</div>
                                            </div>

                                            <div class="ip-form-group">
                                                <label class="ip-label" for="scm_category_id">
                                                    Item Category <span class="required">*</span>
                                                </label>
                                                @php $catRows = collect($lists['category'] ?? []); @endphp
                                                <select id="scm_category_id" name="category_id" class="ip-field ip-select">
                                                ...
                                                <div class="ip-help">Item category from cr_item_categories (tenant scoped).</div>
                                            </div>

                                            <div class="ip-form-group">
                                                <label class="ip-label" for="scm_sort_order">Sort Order</label>
                                                <input type="number"
                                                       id="scm_sort_order"
                                                       name="sort_order"
                                                       class="ip-field"
                                                ...
                                                <div class="ip-help">Controls display order in the UI (lower first).</div>
                                            </div>

                                            <div class="ip-form-group">
                                                <label class="ip-label" for="scm_is_active">Status</label>
                                                <div class="ip-switch">
                                                    <input type="hidden" name="is_active" value="0">
                                                    <input type="checkbox"
                                                           id="scm_is_active"
                                                    ...
                                                    <span class="ip-switch-label">Active</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
(function () {
    ...
    // PATCH: Disable inputs in non-active entity sections to prevent hidden-field submission bugs
    function setEntityInputsEnabled(entityKey) {
        entityForms.forEach(function (section) {
            var active = section.getAttribute('data-entity') === entityKey;
            section.querySelectorAll('input, select, textarea').forEach(function (el) {
                // Safety: only entity-section fields are toggled here
                el.disabled = !active;
            });
        });
    }
...
function showAlert(type, message) {
    ...
    function getRequiredFieldIds(entityKey) {
        switch (entityKey) {
            ...
            case 'space_category_mapping': return ['scm_space_id', 'scm_category_id'];
            default: return [];
        }
    }
...
function clearAllPreviewEls() {
        // Clear preview containers in the UI (front-end only)
        setPreviewEl('pt_image_preview',  '', 'No image selected yet. A clean, thematic thumbnail works best.');
        setPreviewEl('pst_image_preview', '', 'Sub-type illustration used in project selection cards.');
        setPreviewEl('sp_image_preview',  '', 'Used in “Spaces in this Requisition” pills/cards.');
        setPreviewEl('cat_image_preview', '', 'Thumbnail shown in Item Category cards in Step-2.');
        setPreviewEl('sub_image_preview', '', 'Used in “Sub-Categories” slider for the selected category.');
        setPreviewEl('prod_image_preview','', 'Used as hero image in the Product cards slider.');
    }
    ...
    bindImagePreview('sp_image',   'sp_image_preview',   'Used in “Spaces in this Requisition” pills/cards.');
    bindImagePreview('cat_image',  'cat_image_preview',  'Thumbnail shown in Item Category cards in Step-2.');
    bindImagePreview('sub_image',  'sub_image_preview',  'Used in “Sub-Categories” slider for the selected category.');
    ...
    function restoreStoredPreviewsIfNeeded() {
        ...
        var map = [
            ...
            { input: 'sp_image',   preview: 'sp_image_preview',   ph: 'Used in “Spaces in this Requisition” pills/cards.' },
            { input: 'cat_image',  preview: 'cat_image_preview',  ph: 'Thumbnail shown in Item Category cards in Step-2.' },
            { input: 'sub_image',  preview: 'sub_image_preview',  ph: 'Used in “Sub-Categories” slider for the selected category.' },
    ...
    function activateEntity(entityKey, opts) {
        ...
        setEntityInputsEnabled(currentEntity);
var meta = entityMeta[currentEntity] || entityMeta.project_type;
    ...
    function hydrateFromRecord(btn) {
        ...
        if (entity === 'space') {
            ...
            setPreviewFromUrl('sp_image_preview', btn.dataset.imageUrl, 'Used in “Spaces in this Requisition” pills/cards.');
        }
        ...
        if (entity === 'subcategory') {
            ...
            setPreviewFromUrl('sub_image_preview', btn.dataset.imageUrl, 'Used in “Sub-Categories” slider for the selected category.');
        }
        ...
        if (entity === 'space_category_mapping') {
            var spSel = document.getElementById('scm_space_id');
            var catSel = document.getElementById('scm_category_id');
            if (spSel) spSel.value = btn.dataset.spaceId || '';
            if (catSel) catSel.value = btn.dataset.categoryId || '';

            document.getElementById('scm_sort_order').value  = btn.dataset.sortOrder || '';
            document.getElementById('scm_is_active').checked = btn.dataset.isActive !== '0';
        }
    ...
    if (btnDelete) {
        btnDelete.addEventListener('click', function () {
            ...
            var url = baseUrl + '/' + encodeURIComponent(id) + '?_entity=' + encodeURIComponent(currentEntity || '');
            ...
            .then(function (res) {
                // 204 No Content is a valid successful delete
                if (res && res.status === 204) return { __ok: true };
                ...
                if (ct.indexOf('application/json') !== -1) {
                    return res.json().catch(function () { return { __ok: !!res.ok }; });
                }

                // If server returned HTML (redirect/error page), treat by HTTP status
                return res.text()
                    .then(function () { return { __ok: !!res.ok }; })
                    .catch(function () { return { __ok: !!res.ok }; });
            })
            .then(function (json) {
                var ok = (json && (json.ok === true || json.success === true || json.__ok === true));
                if (ok) {
