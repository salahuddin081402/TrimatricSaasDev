{{-- resources/views/registration/professional/edit.blade.php --}}
...
@php
  ...
  $existingNF    = $row->NID_Photo_Front_Page ?? '';
  $existingNB    = $row->NID_Photo_Back_Page ?? '';
  $existingBC    = $row->Birth_Certificate_Photo ?? '';
@endphp

<div class="container py-3">
  <h1 class="h4 tmx-title mb-3 d-flex justify-content-between align-items-center">
    <span><i class="fa-solid fa-user-tie me-2"></i> Edit Professional Registration — {{ $company->name }}</span>
  ...
  <form id="proCreateForm" method="POST"
        action="{{ route('registration.professional.update', ['company' => $companyParam]) }}"
    ...
    <div class="tmx-card mb-3" id="card-personal">
      <div class="tmx-card-hd">
        ...
        <div class="h-note">Update as needed. Locked fields come from your account.</div>
      </div>
        ...
        <div class="row g-3">
          <div class="col-12 col-md-4">
            ...
            <div class="d-flex flex-wrap gap-2">
              @php $genderOld = old('gender', $row->gender); @endphp
          ...
          <div class="col-12 col-lg-3">
            ...
            <select id="division_id" name="division_id" class="form-select @error('division_id') is-invalid @enderror">
              ...
              @foreach($divisions as $d)
                @php $sel = old('division_id', $row->division_id); @endphp
                <option value="{{ $d->id }}" {{ $sel == $d->id ? 'selected' : '' }}>
                  {{ $d->short_code }} — {{ $d->name }}
                </option>
              @endforeach
            </select>
          ...
          <div class="col-12 col-md-6">
            ...
            @php $types = ['J'=>'Service Holder','B'=>'Business Person','H'=>'Housewife','S'=>'Student','P'=>'Professional','O'=>'Other']; $ptypeOld = old('person_type', $row->person_type); @endphp
            <div class="d-flex flex-wrap gap-2">
              @foreach ($types as $k => $v)
                <label class="form-check form-check-inline">
                  <input class="form-check-input @error('person_type') is-invalid @enderror" type="radio"
                         name="person_type" value="{{ $k }}" {{ $ptypeOld == $k ? 'checked' : '' }}>
                  <span class="form-check-label">{{ $v }}</span>
                </label>
              @endforeach
            </div>
          ...
          <div class="col-12 col-md-6">
            ...
            @php $profOld = (int) old('profession', (int)($row->profession_id ?? 0)); @endphp
            <div class="d-flex flex-wrap gap-3">
              @foreach ($professions as $p)
                <label class="form-check form-check-inline">
                  <input class="form-check-input @error('profession') is-invalid @enderror" type="radio"
                         name="profession" value="{{ $p->id }}" {{ $profOld === (int)$p->id ? 'checked' : '' }}>
          ...
          <div class="col-12 col-lg-4">
            ...
            @php $pTemp = old('temp_photo', $temps['temp_photo'] ?? ''); $pShow = $pTemp ?: ($existingPhoto ? asset($existingPhoto) : ''); @endphp
            <div class="preview" id="preview_photo" @if(empty($pShow)) hidden @endif>
              @if($pShow)
                <span class="thumb">
                  <img src="{{ $pShow }}" alt="photo">
          ...
          <div class="col-12 col-lg-4">
            ...
            <div class="row g-2 mt-1">
              <div class="col-6">
                ...
                @php $nfTemp = old('temp_nid_front', $temps['temp_nid_front'] ?? ''); $nfShow = $nfTemp ?: ($existingNF ? asset($existingNF) : ''); @endphp
                <div class="preview" id="preview_nid_front" @if(empty($nfShow)) hidden @endif>
                  @if($nfShow)
                    <span class="thumb">
                      <img src="{{ $nfShow }}" alt="nid_front">
              ...
              <div class="col-6">
                ...
                @php $nbTemp = old('temp_nid_back', $temps['temp_nid_back'] ?? ''); $nbShow = $nbTemp ?: ($existingNB ? asset($existingNB) : ''); @endphp
                <div class="preview" id="preview_nid_back" @if(empty($nbShow)) hidden @endif>
                  @if($nbShow)
                    <span class="thumb">
                      <img src="{{ $nbShow }}" alt="nid_back">
          ...
          <div class="col-12 col-lg-4">
            ...
            @php $bcTemp = old('temp_birth_certificate', $temps['temp_birth_certificate'] ?? ''); $bcShow = $bcTemp ?: ($existingBC ? asset($existingBC) : ''); @endphp
            <div class="preview" id="preview_birth" @if(empty($bcShow)) hidden @endif>
              @if($bcShow)
                <span class="thumb">
                  <img src="{{ $bcShow }}" alt="birth">
    ...
    <div class="tmx-card mb-3" id="card-soft">
      ...
      <div class="tmx-card-bd">
        @php
          ...
          $seedSoftSel  = count($oldSoftSel) ? $oldSoftSel : ($selSoft ?? collect())->all();
        ...
        <div class="scroll-box">
          @forelse($softwares as $s)
            @php
              $isChecked = in_array((int)$s->id, $seedSoftSel, true);
    ...
    <div class="tmx-card mb-3" id="card-skills">
      ...
      <div class="tmx-card-bd">
        ...
        <div class="scroll-box">
          @php
            $oldSkills = collect(old('skill_ids', []))->map(fn($v)=>(int)$v)->all();
            $seedSkills = count($oldSkills) ? $oldSkills : ($selSkills ?? collect())->all();
          @endphp
          @forelse($skills as $sk)
            <div class="form-check">
              <input class="form-check-input @error('skill_ids') is-invalid @enderror" type="checkbox" name="skill_ids[]"
                     value="{{ $sk->id }}" id="sk_{{ $sk->id }}" {{ in_array((int)$sk->id, $seedSkills, true) ? 'checked' : '' }}>
    ...
    <div class="tmx-card mb-3" id="card-areas">
      ...
      <div class="tmx-card-bd">
        ...
        <div class="scroll-box">
          @php
            $oldTasks = collect(old('task_ids', []))->map(fn($v)=>(int)$v)->all();
            $seedTasks = count($oldTasks) ? $oldTasks : ($selTasks ?? collect())->all();
          @endphp
          @forelse($tasks as $t)
            <div class="form-check">
              <input class="form-check-input" type="checkbox" name="task_ids[]"
                     value="{{ $t->Task_Param_ID }}" id="tk_{{ $t->Task_Param_ID }}" {{ in_array((int)$t->Task_Param_ID, $seedTasks, true) ? 'checked' : '' }}>
    ...
    <div class="tmx-card mb-4" id="card-confirm">
      ...
      <div class="tmx-card-bd">
        ...
        <div class="d-flex flex-wrap gap-2 justify-content-end mt-3">
          ...
          <button type="button" id="btnCancel" class="btn btn-x btn-cancel"><i class="fa-solid fa-broom"></i> Cancel (Reset Changes)</button>
          <button type="button" id="btnValidateSave" class="btn btn-x btn-save"><i class="fa-solid fa-paper-plane"></i> Validate &amp; Update</button>
        </div>
      </div>
    </div>
...
@php
  // Seed arrays once
  $seedEdu = $eduRows->map(function($r){
    return [
      'degree_id'   => $r->degree_id,
      'institution' => $r->Institution,
      'passing_year'=> $r->Passing_Year,
      'result_type' => $r->Result_Type,
      'score'       => $r->obtained_grade_or_score,
      'out_of'      => $r->Out_of,
    ];
  })->values();

  $seedJob = $jobRows->map(function($r){
    return [
      'employer'   => $r->Employer,
      'job_title'  => $r->Job_title,
      'join_date'  => $r->Joining_date,
      'is_present' => $r->is_present_job,
      'end_date'   => $r->End_date,
    ];
  })->values();

  $seedTrn = $trainRows->map(function($r){
    return [
      'category_id' => $r->Training_Category_Id,
...
(function () {
  ...
  const EXIST = {
    photo: @json((bool)$existingPhoto),
    nid_front: @json((bool)$existingNF),
    nid_back: @json((bool)$existingNB),
    birth: @json((bool)$existingBC),
    urls: {
      photo: @json($existingPhoto ? asset($existingPhoto) : ''),
      nid_front: @json($existingNF ? asset($existingNF) : ''),
      nid_back: @json($existingNB ? asset($existingNB) : ''),
      birth: @json($existingBC ? asset($existingBC) : ''),
    }
  };
  ...
  (async function restoreGeo(){
    ...
    const seedDivision = oldDivision || @json($row->division_id);
    const seedDistrict = oldDistrict || @json($row->district_id);
    const seedUpazila  = oldUpazila  || @json($row->upazila_id);
    const seedThana    = oldThana    || @json($row->thana_id);

    if(seedDivision) tsSetValue(divisionEl,seedDivision);
    await loadDistricts(seedDivision, seedDistrict || null);
    if(seedDistrict){
      await loadUpazilas(seedDistrict, seedUpazila || null);
      await loadThanas(seedDistrict, seedThana || null);
    }
  })();
  ...
  // single, safe definitions for old()/seed arrays
  const OLD_EDU = @json(old('edu', []));
  const OLD_JOB = @json(old('job', []));
  const OLD_TRN = @json(old('train', []));
  const SEED_EDU = @json($seedEdu);
  const SEED_JOB = @json($seedJob);
  const SEED_TRN = @json($seedTrn);

  // Restore dynamic rows from old() or from server seeds
  (async function restoreDynamic(){
    const useEdu = Array.isArray(OLD_EDU) && OLD_EDU.length ? OLD_EDU : (Array.isArray(SEED_EDU) && SEED_EDU.length ? SEED_EDU : []);
    const useJob = Array.isArray(OLD_JOB) && OLD_JOB.length ? OLD_JOB : (Array.isArray(SEED_JOB) && SEED_JOB.length ? SEED_JOB : []);
    const useTrn = Array.isArray(OLD_TRN) && OLD_TRN.length ? OLD_TRN : (Array.isArray(SEED_TRN) && SEED_TRN.length ? SEED_TRN : []);

    if(useEdu.length){
      eduWrap.innerHTML='';
      useEdu.forEach((r,i)=>{ const d=document.createElement('div'); d.innerHTML=eduRowHtml(i); const n=d.firstElementChild; eduWrap.appendChild(n); mountTS(n);
    ...
    if(useJob.length){
      jobWrap.innerHTML='';
      useJob.forEach((r,i)=>{ const d=document.createElement('div'); d.innerHTML=jobRowHtml(i); const n=d.firstElementChild; jobWrap.appendChild(n); mountTS(n); mountJobCalendars(n);
  ...
  function bindUploadAndPreview(inputId, previewId, fieldName, existingUrl=''){
    ...
    const current = (hidden && hidden.value) ? hidden.value : (existingUrl || '');
    if(current){
      box.innerHTML = `<span class="thumb"><img src="${current}" alt="${fieldName}"><button type="button" class="rmv" data-clear="${fieldName}">Remove</button></span>`;
      box.hidden=false;
    }
  ...
  bindUploadAndPreview('photo','preview_photo','photo', EXIST.urls.photo);
  bindUploadAndPreview('nid_front','preview_nid_front','nid_front', EXIST.urls.nid_front);
  bindUploadAndPreview('nid_back','preview_nid_back','nid_back', EXIST.urls.nid_back);
  bindUploadAndPreview('birth_certificate','preview_birth','birth_certificate', EXIST.urls.birth);
  ...
  function validateAll(){
    ...
    if(!photoFile && !photoTemp && !EXIST.photo){ okPersonal=false; const ph=document.getElementById('photo'); if(!firstBad) firstBad=ph; addFieldError(ph,'Photo is required.'); }

    const nidNumInput=(document.getElementById('nid_number').value||'').trim();
    const nidEffective = nidNumInput.length ? nidNumInput : '';
    ...
    if(nidEffective.length){
      if(!NID_ALLOWED.includes(nidEffective.length)){ okPersonal=false; const nid=document.getElementById('nid_number'); if(!firstBad) firstBad=nid; addFieldError(nid,'NID length must be one of: '+NID_ALLOWED.join(', ')); const live=document.getElementById('nid_len_msg'); if(live){ live.style.display='block'; } }
      if(!nfFile && !nfTemp && !EXIST.nid_front){ okPersonal=false; const nf=document.getElementById('nid_front'); if(!firstBad) firstBad=nf; addFieldError(nf,'Provide NID front image.'); }
      if(!nbFile && !nbTemp && !EXIST.nid_back){ okPersonal=false; const nb=document.getElementById('nid_back'); if(!firstBad) firstBad=nb; addFieldError(nb,'Provide NID back image.'); }
    }else{
      if(!bcFile && !bcTemp && !EXIST.birth){ okPersonal=false; const bc=document.getElementById('birth_certificate'); if(!firstBad) firstBad=bc; addFieldError(bc,'Birth certificate is required if NID not provided.'); }
    }
