...
<style>
  /* ===== Elegant palette (high-contrast, mobile-first) ===== */
  :root{
    --ink:#2b2ba1cf;                /* headings, labels */
    --sub:#6b7280;                  /* muted text */
    --iconbox:#eef1ff;              /* icon chip */
    --card-grad-1:#faf7ff;          /* soft ivory-lilac */
    --card-grad-2:#f4f7ff;          /* soft icy-blue */
    --card-border:#e6e7f2;
    --card-shadow:0 12px 28px rgba(31,42,68,.08);

    --rec-grad-1:#f6f9ff;           /* record capsule bg */
    --rec-grad-2:#f9f6ff;
    --rec-border:#dbe2ff;

    --static-bg:#f7f6f9;            /* static value chips */
    --static-br:#e6e1f0;

    --scroll-bg:#fbfbfe;            /* scrollable lists */
    --scroll-br:#e7e9f5;
    ...
    --label: var(--ink);            /* keep legacy var usage */
    --soft-bg:#f8fafc;              /* legacy fallback */
    --rec-bg:#f6f9ff;               /* legacy fallback */
  }
  ...
  .tmx-iconbox{
    ...
    width:34px;height:34px;background:var(--iconbox);
  ...
  .tmx-static{
    ...
    border:1px solid var(--static-br);
    border-radius:10px;padding:10px 12px;
    display:flex;align-items:center;gap:8px;
  }

  /* ===== Scrollable lists (skills, areas, software) ===== */
  .scroll-box{
    border:1px solid var(--scroll-br);
    border-radius:12px;
    padding:16px 18px;max-height:260px;overflow:auto;
    background:linear-gradient(180deg,#ffffff 0%, var(--scroll-bg) 100%);
  }
  .scroll-box .form-check{
    padding:12px 8px;border-bottom:1px dashed #eef2f7;
    display:flex;align-items:center;gap:12px;
  }
  .scroll-box .form-check:last-child{border-bottom:none;}

  /* ===== Record capsule (each dynamic row) ===== */
  .rec{
    background:linear-gradient(180deg,var(--rec-grad-1),var(--rec-grad-2));
  ...
  .date-wrap .fa-regular.fa-calendar-days{
  ...
  .preview img{
    width:100%;max-width:220px;max-height:160px;object-fit:contain;
    border-radius:10px;border:1px solid var(--card-border);cursor:zoom-in;background:#fff;
    box-shadow:0 8px 16px rgba(31,42,68,.06);
  }
  .preview .rmv{
    position:absolute;top:6px;right:6px;background:rgba(0,0,0,.6);
    color:#fff;border:none;border-radius:6px;font-size:.75rem;padding:2px 6px;cursor:pointer
  }

  /* keep button colors stable on hover/focus/active */
  .btn-x, .btn-x:hover, .btn-x:focus, .btn-x:active { background: var(--bg) !important; color: var(--fg) !important; }
  /* neutralize Bootstrap outline hover flips in this view */
  .btn, .btn:hover, .btn:focus, .btn:active { text-decoration:none; }

  /* ===== Alerts ===== */
  .v-alert{
    display:none;border:1px solid #fecaca;background:#fef2f2;color:#991b1b;
    border-radius:12px;padding:10px 12px;margin-bottom:12px;
    box-shadow:0 10px 20px rgba(153,27,27,.05);
  }

  @media (max-width:576px){
    .tmx-card-bd{padding:12px}
    .tmx-card-hd{padding:12px}
  }
</style>
@endpush
...
<div class="container py-3">
  <h1 class="h4 tmx-title mb-3 d-flex justify-content-between align-items-center">
    <span><i class="fa-solid fa-user-tie me-2"></i> Company Officer Registration — {{ $company->name }}</span>
  ...
  <form id="coCreateForm" method="POST"
        action="{{ route('registration.company_officer.store', ['company' => $companyParam]) }}"
        ...
        <div class="row g-3 mb-1">
          ...
          <div class="col-12 col-lg-4">
            ...
            <div class="tmx-static">
              ...
              <span>{{ $ctx['reg_key'] }}</span>
            </div>
            <input type="hidden" name="reg_key" value="{{ old('reg_key', $ctx['reg_key']) }}">
          </div>
        </div>
        ...
        <div class="row g-3">
          <div class="col-12 col-md-6">
            ...
            <input type="email" class="form-control @error('email') is-invalid @enderror" name="email"
                   value="{{ old('email', $ctx['email']) }}"
                   placeholder="Enter official email from appointment letter" autocomplete="email">
            <div class="form-text">Use the email provided by Head Office (appointment letter).</div>
          ...
          <div class="col-12 col-md-6">
            ...
            <input type="text" class="form-control @error('employee_id') is-invalid @enderror" name="employee_id"
                   value="{{ old('employee_id') }}" placeholder="Employee ID from appointment letter" autocomplete="off">
          ...
          <div class="col-12 col-lg-4">
            ...
            <div class="preview" id="preview_photo" @if(empty($temps['temp_photo'])) hidden @endif>
              @if(!empty($temps['temp_photo']))
                <span class="thumb">
                  <img src="{{ $temps['temp_photo'] }}" alt="photo">
          ...
          <div class="col-12 col-lg-4">
            ...
            <div class="row g-2 mt-1">
              <div class="col-6">
                ...
                <div class="preview" id="preview_nid_front" @if(empty($temps['temp_nid_front'])) hidden @endif>
                  @if(!empty($temps['temp_nid_front']))
                    <span class="thumb">
                      <img src="{{ $temps['temp_nid_front'] }}" alt="nid_front">
              ...
              <div class="col-6">
                ...
                <div class="preview" id="preview_nid_back" @if(empty($temps['temp_nid_back'])) hidden @endif>
                  @if(!empty($temps['temp_nid_back']))
                    <span class="thumb">
                      <img src="{{ $temps['temp_nid_back'] }}" alt="nid_back">
            ...
            <div id="nid_hint" class="form-text mt-1">
              Accepted NID lengths: {{ implode(', ', $nidLengths) }} (config-driven).
            </div>
          </div>
          ...
          <div class="col-12 col-lg-4">
            ...
            <div class="preview" id="preview_birth" @if(empty($temps['temp_birth_certificate'])) hidden @endif>
              @if(!empty($temps['temp_birth_certificate']))
                <span class="thumb">
                  <img src="{{ $temps['temp_birth_certificate'] }}" alt="birth">
      ...
      <div class="tmx-card-bd">
        ...
        <div class="d-flex flex-wrap gap-2 justify-content-end mt-3">
          ...
          <button type="button" id="btnCancel" class="btn btn-x btn-cancel">
            <i class="fa-solid fa-broom"></i> Cancel (Clear Unsaved Data)
          </button>
          <button type="button" id="btnValidateSave" class="btn btn-x btn-save">
            <i class="fa-solid fa-paper-plane"></i> Validate &amp; Save All
          </button>
        </div>
      </div>
    </div>
...
<script>
(function () {
  const form       = document.getElementById('coCreateForm');
  ...
  /* ---------- server-error autofocus for skills/areas ---------- */
  ...
  /* ---------- Change handlers (job present -> end date) ---------- */
  ...
  /* ---------- Shared calendar for Job Experience fields ---------- */
  ...
  function trainRowHtml(idx){
    ...
    return `
      <div class="rec" data-trn-idx="${idx}">
        <div class="row g-2">
          <div class="col-12 col-md-4"><label class="tmx-label">Training Category *</label><select name="train[${idx}][category_id]" class="form-select tmx-ts-single trn-cat"><option value="" hidden>— Select —</option>${cat}</select></div>
          <div class="col-12 col-md-4"><label class="tmx-label">Training Name *</label><select name="train[${idx}][training_id]" class="form-select tmx-ts-single trn-name" disabled><option value="" hidden>— Select category first —</option></select></div>
          <div class="col-12 col-md-4"><label class="tmx-label">Remarks</label><input type="text" name="train[${idx}][remarks]" class="form-control"/></div>
        </div>
        <div class="rec-actions">
  ...
  (async function restoreDynamic(){
    ...
    if(Array.isArray(oldTrn)&&oldTrn.length){
      ...
      for(let i=0;i<oldTrn.length;i++){
        ...
        const cat=n.querySelector(`[name="train[${i}][category_id]"]`), nameSel=n.querySelector(`[name="train[${i}][training_id]"]`), rem=n.querySelector(`[name="train[${i}][remarks]"]`);
        tsSetValue(cat, r.category_id??''); rem.value=r.remarks??'';
        if(r.category_id){ await loadTrainings(cat); tsSetValue(nameSel, r.training_id??''); }
      }
    }
  })();

  /* ---------- Training names loader ---------- */
  ...
  function validateAll(){
    ...
    if(nidNum.length){
      if(!@json($nidLengths).includes(nidNum.length)){ okPersonal=false; const nid=document.getElementById('nid_number'); if(!firstBad) firstBad=nid; addFieldError(nid,'NID length must be one of: '+@json($nidLengths).join(', ')); }
    ...
    // focus first checkbox when missing
    ...
    // TRAIN (optional; remarks optional)
    let okTrain=true;
    document.querySelectorAll('[data-trn-idx]').forEach(b=>{
      const cat=b.querySelector('select[name*="[category_id]"]'), trn=b.querySelector('select[name*="[training_id]"]'), rem=b.querySelector('input[name*="[remarks]"]');
      const touched = tsGetValue(cat) || tsGetValue(trn) || (rem && rem.value.trim());
      if(!touched) return;
