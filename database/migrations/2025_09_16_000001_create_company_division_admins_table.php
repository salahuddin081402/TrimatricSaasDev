...
return new class extends Migration {
    ...
    {
        Schema::create('company_division_admins', function (Blueprint $table) {
            ...
            // Uniques / indexes
            $table->unique(['company_id', 'division_id', 'user_id'], 'uq_cda_user_area');
            $table->index(['company_id', 'division_id', 'status'], 'idx_cda_area_status');
            $table->index(['company_id', 'user_id'], 'idx_cda_user');
            $table->index('division_id', 'idx_cda_div');
            $table->index('status', 'idx_cda_status');
            $table->index('created_by', 'idx_cda_created_by');
            $table->index('updated_by', 'idx_cda_updated_by');
        });

        // Optional CHECK (MySQL 8+); safe if ignored by engine
        DB::unprepared("
            ALTER TABLE company_division_admins
            ADD CONSTRAINT chk_cda_status CHECK (status IN (0,1))
        ");
...
SQL);
    ...
    {
        // Drop triggers first
        DB::unprepared('DROP TRIGGER IF EXISTS trg_company_division_admins_bi');
        DB::unprepared('DROP TRIGGER IF EXISTS trg_company_division_admins_bu');

        Schema::dropIfExists('company_division_admins');
    }
};
